	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (mTouchState == TOUCH_STATE_RESTART) {
				...
				if (((x <= mWidth&&RightState ==true )||
						(x >= screenWidth - mWidth&&LeftState ==true ))
		...
		case MotionEvent.ACTION_MOVE:
			...
			if ((mScrollState == SCROLL_STATE_ALLOW&&LeftState ==true
					&& getWidth() - (int) ev.getX() < mWidth)||
					(mScrollState == SCROLL_STATE_ALLOW&&RightState ==true
							&& (int) ev.getX() < mWidth)){
				return true;
			}
			break;
		...
		case MotionEvent.ACTION_UP:
			if (mScrollState == SCROLL_STATE_ALLOW) {
				if (mVelocityValue > 2000  ) {
					if(LeftState ==true&&mScreenState == SCREEN_STATE_CLOSE){
						mScreenState = SCREEN_STATE_OPEN;
						mScroller.startScroll(getChildAt(1).getScrollX(),0,-(getWidth()
						- Math.abs(getChildAt(1).getScrollX()) -mWidth), 0, 250);
						invalidate();
						}
					else  if (RightState =true&&mScreenState == SCREEN_STATE_OPEN ){
						mScreenState = SCREEN_STATE_CLOSE;
						mScroller.startScroll(getChildAt(1).getScrollX(), 0,
					   ...
					   if(LeftState ==true &&mScreenState == SCREEN_STATE_OPEN){
							mScreenState = SCREEN_STATE_CLOSE;
							mScroller.startScroll(getChildAt(1).getScrollX(), 0,
									-getChildAt(1).getScrollX(), 0, 250);
							invalidate();
							}
						else  if (RightState =true &&mScreenState == SCREEN_STATE_CLOSE){
							mScreenState = SCREEN_STATE_OPEN;
							mScroller.startScroll(getChildAt(1).getScrollX(),0,(getWidth()
							- Math.abs(getChildAt(1).getScrollX()) -mWidth), 0, 250);
							invalidate();
					...
					mScroller
							.startScroll(
									getChildAt(1).getScrollX(),0,
									-(getWidth()- Math.abs(getChildAt(1).getScrollX()) -mWidth), 0, 800);
					invalidate();
					}
