...
public final class MultiThreadDownload implements Runnable {  
    ...
    /* 用于监视下载状态 */  
    private final DownloadUtil downloadService;  
    /* 线程下载任务的起始点 */  
    public int start;  
    /* 线程下载任务的结束点 */  
    private int end;  
  
    public MultiThreadDownload(int id, File savedFile, int block, String path, Integer downlength, DownloadUtil downloadService) throws Exception {  
        this.id = id;  
        this.path = path;  
    ...
    public void run() {  
        try {  
            ...
            in.close();  
            conn.disconnect();  
            if (!downloadService.isPause) Log.i(DownloadUtil.TAG, "Thread " + (this.id + 1) + "finished");  
            finished = true;  
        } catch (Exception e) {  
            e.printStackTrace();  
