...
import cn.dennishucd.anim.PhotoAnimations;
import cn.dennishucd.cache.BaseActivity;
import cn.dennishucd.result.PhotoResult;
import cn.dennishucd.uibase.PhotoGallery;
import cn.dennishucd.uibase.PhotoImageView;
 ...
 * 
 */
public class PhotoAvatarDetailActivity extends BaseActivity {
	...
	private void setListener() {
		...
		mSaveas.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// 获取照片位置
				String avatar = mPhotoResult.getImages().get(mCurrentPosition)
						.getImage();
				Bitmap bitmap = null;
				// 根据编号获取头像
				bitmap = mKXApplication.getAvatar(avatar);
				// 保存该头像到本地SD卡
				String result = PhotoUtil.saveToSDCard(bitmap);
				if (result!=null) {
					Toast.makeText(PhotoAvatarDetailActivity.this,
							"已保存到/DCIM/ImageLife/download/下", Toast.LENGTH_SHORT)
							.show();
				} else {
					Toast.makeText(PhotoAvatarDetailActivity.this,
	...
	private class PhotoGalleryAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			// 获取头像路径
			String avatar = mPhotoResult.getImages().get(position).getImage();
			Bitmap bitmap = null;
			// 根据路径获取头像
			bitmap = mKXApplication.getAvatar(avatar);
			// 获取缩放比例
